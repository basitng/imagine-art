...
const mockCommunityImages: CommunityItem[] = [
  {
    id: 1,
    imageUrl:
...
export default function CommunityLayout({ userInfo }: { userInfo: UserInfo }) {
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-indigo-50 via-stone-50 to-indigo-100 flex flex-col">
      ...
      <main className="flex-grow w-full max-w-screen-xl mx-auto flex flex-col items-start mt-24 px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-lg mb-8">
          <h3 className="text-3xl font-display">Community</h3>
          <p className="text-gray-500">
            Discover and get inspired by the latest AI-generated artworks
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {mockCommunityImages.map((item) => (
            <CommmunityImage key={item.id} item={item} />
          ))}
        </div>
      </main>
